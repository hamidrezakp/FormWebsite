...
#[get("/")]
async fn get_all(conn: Db) -> Result<Json<Vec<CaseAction>>> {
...
async fn insert(case_action: Json<NewCaseAction>, conn: Db) -> Result<Json<CaseAction>> {
    let case_action = CaseAction::new(&conn, case_action.into_inner()).await?;
    Ok(Json(case_action))
}

#[put("/", data = "<case_action>")]
async fn update(case_action: Json<CaseAction>, conn: Db) -> Result<()> {
